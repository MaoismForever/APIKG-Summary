...
class EngineFactory:
    ...
    def create_engine_to_center(echo=True):
        engine = create_engine("mysql+pymysql://root:root@00.00.00.00/domainkg?charset=utf8", encoding='utf-8',
                               echo=echo)
        return engine
    ...
    def create_engine_by_schema_name(schema_name, echo=True):
        if schema_name == 'stackoverflow':
            engine = create_engine("mysql+pymysql://root:root@00.00.00.00/stackoverflow?charset=utf8",
                                   encoding='utf-8',
                                   echo=echo)
            return engine
        elif schema_name == 'knowledgeGraph':
            engine = create_engine("mysql+pymysql://root:root@00.00.00.00/knowledgeGraph?charset=utf8",
                                   encoding='utf-8',
                                   echo=echo)
            return engine
        elif schema_name == 'api_backup':
            engine = create_engine("mysql+pymysql://root:root@00.00.00.00/api_backup?charset=utf8mb4", encoding='utf-8',
                                   echo=echo)
            return engine
            ...
            engine = create_engine("mysql+pymysql://root:root@00.00.00.00/ner?charset=utf8", encoding='utf-8',
                                   echo=echo)
            return engine
        else:
            return None
